...
using android::hardware::light::V2_0::implementation::DeviceInfo;
...
int main() {
    DeviceInfo deviceType = android::hardware::light::V2_0::implementation::DEVICE_UNKNOWN;
    uint32_t lcdMaxBrightness = 255;
    std::string lcdBacklightPath;
    std::string kLcdMaxBacklightPath;
    ...
    if (!deviceInfoStream) {
        ...
        deviceInfo << deviceInfoStream.rdbuf();
        if (startswith("le_zl0", deviceInfo.str())) {
            LOG(WARNING) << "DeviceInfo: ZL0";
            deviceType = android::hardware::light::V2_0::implementation::DEVICE_ZL0;
            lcdBacklightPath = kLcdBacklightPath;
            kLcdMaxBacklightPath = kLcdMaxBacklightPath;
            hasRGBlight = true;
        } else if (startswith("le_zl1", deviceInfo.str())) {
            LOG(WARNING) << "DeviceInfo: ZL1";
            deviceType = android::hardware::light::V2_0::implementation::DEVICE_ZL1;
            lcdBacklightPath = kLcdBacklightPath;
            kLcdMaxBacklightPath = kLcdMaxBacklightPath;
            hasRGBlight = true;
        } else if (startswith("le_x2", deviceInfo.str())) {
            LOG(WARNING) << "DeviceInfo: X2";
            deviceType = android::hardware::light::V2_0::implementation::DEVICE_X2;
            lcdBacklightPath = kWledBacklightPath;
            kLcdMaxBacklightPath = kWledMaxBacklightPath;
        } else {
    ...
    if (!lcdBacklight) {
        LOG(ERROR) << "Failed to open " << lcdBacklightPath << ", error=" << errno
                   << " (" << strerror(errno) << ")";
        return -errno;
    }
