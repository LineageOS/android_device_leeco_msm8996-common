...
public class ConsumerirTransmitterService extends Service {
   ...
   */
  private final ServiceConnection mControlServiceConnection =
  ...
  public void bindQuickSetService() {
    if (DEBUG)
      Log.d(TAG, "Trying to bind QuickSet service: " +
                     IControl.QUICKSET_UEI_PACKAGE_NAME + " - " +
                     IControl.QUICKSET_UEI_SERVICE_CLASS);
    try {
      Intent controlIntent = new Intent(IControl.ACTION);
      controlIntent.setClassName(IControl.QUICKSET_UEI_PACKAGE_NAME,
                                 IControl.QUICKSET_UEI_SERVICE_CLASS);
      boolean bindResult = bindService(controlIntent, mControlServiceConnection,
  ...
  public int transmitIrPattern(int carrierFrequency, int[] pattern) {
    if (DEBUG)
      Log.d(TAG, "transmitIrPattern called: freq: " + carrierFrequency +
    ...
    if (mControl == null || !mBound) {
      if (DEBUG)
        Log.w(
            TAG,
            "QuickSet Service seems not to be bound. Trying to bind again and exit!");
    ...
    try {
      ...
      Log.e(TAG, "Exception while trying to send command to QuickSet Service!",
            t);
      return -1;
    }
  }
      ...
      if (ACTION_TRANSMIT_IR.equals(action)) {
        if (intent.getStringExtra("carrier_freq") != null &&
          ...
          int carrierFrequency =
              Integer.parseInt(intent.getStringExtra("carrier_freq"));
